...
1. Use the Glob tool to check if `~/.0ne/skills/CORE/SKILL.md` exists.

2. **If the file exists:** Read `~/.0ne/skills/CORE/SKILL.md` and follow those instructions to fulfill the user's request. You are done with this file — the full skill has everything needed.
...
**CORE** is a premium skill from [0neOS](https://0neos.com).
...
3. **Activate this skill:**
   ```bash
   bun ~/.0ne/0ne/Tools/skill-install.ts install core
   ```
